...
struct FetchNearestMealImageIntent: AppIntent {
    ...
    func perform() async throws -> some ReturnsValue<MealEntity> & ProvidesDialog & ShowsSnippetView {
        ...
        return .result(value: entity, dialog: "오늘 \(mealType.label) 식단입니다.") {
            MealShareNearestExportView(meal: meal, mealType: mealType)
        }
    }
}

struct FetchNearestMealTextIntent: AppIntent {
    static let title: LocalizedStringResource = "가장 가까운 식단 조회 (텍스트)"
    static let description = IntentDescription("현재 시각 기준으로 오늘 중식 또는 석식 메뉴를 텍스트로 가져옵니다.")

    func perform() async throws -> some ReturnsValue<String> & ProvidesDialog {
        let (meal, mealType) = try AppGroupMealCache.resolveNearestMeal()
        let text = MealShareFormatter.text(for: meal, mealType: mealType)
        return .result(value: text, dialog: "오늘 \(mealType.label) 식단입니다.")
    }
}
