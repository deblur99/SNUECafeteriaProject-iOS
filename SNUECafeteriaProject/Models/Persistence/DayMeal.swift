...
extension DayMeal {
    static func sample() -> [DayMeal] {
        CachedDayMeal.sample().map { cached in
            DayMeal(
                date: cached.date,
                lunchItems: cached.sortedLunchItems.map { MenuItem(name: $0.name, sortIndex: $0.sortIndex) },
                dinnerItems: cached.sortedDinnerItems.map { MenuItem(name: $0.name, sortIndex: $0.sortIndex) },
